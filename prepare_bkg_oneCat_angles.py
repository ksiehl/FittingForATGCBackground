...
class doFit_wj_and_wlvj:
    ...
    def __init__(self, in_channel, in_mj_min=40, in_mj_max=150, in_mlvj_min=900., in_mlvj_max=options.mlvj_hi, fit_model="ExpN", fit_model_alter="ExpTail"):
        ...
        self.plotsDir = 'plots_%s_%s_%s_%s' %(self.channel,self.wtagger_label,options.mlvj_lo,int(options.mlvj_hi))
        if not os.path.isdir("cards_%s_%s_%s_%s"%(self.channel,self.wtagger_label,options.mlvj_lo,int(options.mlvj_hi))):
                os.system("mkdir cards_%s_%s_%s_%s"%(self.channel,self.wtagger_label,options.mlvj_lo,int(options.mlvj_hi)));
        self.rlt_DIR="cards_%s_%s_%s_%s/"%(self.channel,self.wtagger_label,options.mlvj_lo,int(options.mlvj_hi))
...
#            cMassFit.SaveAs(rlt_file.Data());
    ...
    def get_WJets_mlvj_correction_sb_to_sig(self,label, mlvj_model):
        ...
        if mlvj_model=="Sum_Exp":
            rrv_c1_sb = self.workspace4fit_.var("rrv_c1_ExpN%s_sb_%s"%(label,self.channel));
            ...
            rrv_delta_c1 = RooRealVar("rrv_delta_c1_ExpN%s_%s"%(label,self.channel),"rrv_delta_c1_ExpN%s_%s"%(label,self.channel),
                                      ...
                                      self.workspace4fit_.var("rrv_c1_ExpN%s_sig_%s"%(label,self.channel)).getVal()-rrv_c1_sb.getVal()-4*rrv_c1_sb.getError(),
                                      self.workspace4fit_.var("rrv_c1_ExpN%s_sig_%s"%(label,self.channel)).getVal()-rrv_c1_sb.getVal()+4*rrv_c1_sb.getError() )
            rrv_delta_n1 = RooRealVar("rrv_delta_n1_ExpN%s_%s"%(label,self.channel),"rrv_delta_n1_ExpN%s_%s"%(label,self.channel),
                                      self.workspace4fit_.var("rrv_n1_ExpN%s_sig_%s"%(label,self.channel)).getVal()-rrv_n1_sb.getVal(),
                                      self.workspace4fit_.var("rrv_n1_ExpN%s_sig_%s"%(label,self.channel)).getVal()-rrv_n1_sb.getVal()-4*rrv_n1_sb.getError(),
                                      self.workspace4fit_.var("rrv_n1_ExpN%s_sig_%s"%(label,self.channel)).getVal()-rrv_n1_sb.getVal()+4*rrv_n1_sb.getError() )
            rrv_delta_c2 = RooRealVar("rrv_delta_c2_Exp%s_%s"%(label,self.channel),"rrv_delta_c2_Exp%s_%s"%(label,self.channel),
                                      self.workspace4fit_.var("rrv_c2_Exp%s_sig_%s"%(label,self.channel)).getVal()-rrv_c2_sb.getVal(),
    ...
    def get_mj_and_mlvj_dataset(self,in_file_name, label):# to get the shape of m_lvj,jet_mass="jet_mass_pr"
        ...
        if not options.read_trees:
		    ...
		    #print "theta* cut is ", starval;
		    #print "theta1 cut is ", oneval;
		    #print "theta2 cut is ", twoval;
                ...
                if 'WJets0_' in label:
                        fileOut        = TFile.Open('cards_%s_%s_900_%s/datasets_%s_%s.root'%(self.channel,self.wtagger_label,int(options.mlvj_hi),self.channel,self.wtagger_label),'recreate')
                else:
                        fileOut        = TFile.Open('cards_%s_%s_900_%s/datasets_%s_%s.root'%(self.channel,self.wtagger_label,int(options.mlvj_hi),self.channel,self.wtagger_label),'update')
...
def pre_limit_sb_correction(method, channel, in_mj_min=40, in_mj_max=150, in_mlvj_min=900, in_mlvj_max=options.mlvj_hi, fit_model="ExpN", fit_model_alter="ExpTail"): 
