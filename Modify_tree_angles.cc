...
void fill_tree_with_cuts(TTree &oldtree, TTree &tree, TString ch)
{
	...
	for(unsigned int i=0; i<nEntries; i++)
	{
		...
		if(jet_pt>200. && jet_tau21_PUPPI<0.55 && jet_mass_softdrop_PUPPI<150. && jet_mass_softdrop_PUPPI>40. && W_pt>200. && fabs(deltaR_LeptonWJet)>TMath::Pi()/2 && fabs(deltaPhi_WJetMet)>2. && fabs(deltaPhi_WJetWlep)>2. && nbtag==0 && pfMET>METCUT && passTrigger && fabs(costhetastar) < 0.6)
		{
			tree.Fill();
...
void Modify_tree_angles()
{
	...
	std::cout <<"Reading electron W+jets trees"<<std::endl;
	make_el_trees("ele");
